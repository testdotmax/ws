...
//var_dump( $dengji,$jietime,$addtime );exit;
if ( $results == 1)
{
ob_clean();         
 ...
 $apalaBlack = imagecolorallocatealpha($myImage, 0, 0, 0, 98);


for( $tmpTop=140;$tmpTop<=990;$tmpTop += 160 ){
    for( $tmpLeft=70;$tmpLeft<=700;$tmpLeft += 150 ){
         imagettftext($myImage, 40, $zhuan, $tmpLeft, $tmpTop, $apalaBlack, "heiti.ttf",  "$name");
     }
 }
 $time = $addtime . ' 至 ' . $jietime;
imagettftext($myImage, $size, 0, $cf['zuobianju']?:240, $cf['shangbianju']?:398, ${$yanse}, "simhei.ttf",  $name);
imagettftext($myImage, $size, 0, $cf['zuobianju']?:240, $other_zhengshu_set['weixinshangbianju']?:442, ${$yanse}, "simhei.ttf",  $weixin);
imagettftext($myImage, $other_zhengshu_set['dailifont'], 0, $other_zhengshu_set['dailizuobianju']?:286, $other_zhengshu_set['dailishangbianju']?:538, ${$yanse}, "simhei.ttf",  $dengji);
imagettftext($myImage, $other_zhengshu_set['shijianfont'], 0, $other_zhengshu_set['riqizuobianju']?:256, $other_zhengshu_set['riqishangbianju']?:680, ${$yanse}, "simhei.ttf",  $time);
// imagettftext($myImage, $size, 0, $left, $top, ${$yanse}, "simhei.ttf",  "$msg1");
